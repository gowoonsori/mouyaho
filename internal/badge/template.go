...
var heartTemplate = strings.TrimSpace(`
<html lang="en">
<head>
    <title>likeIt</title>
    <style>
        ...
        .badge-container {display: block; background-color:{{.BgColor}};}
        .badge-container .badge-box {border: 2px solid {{.BorderColor}};box-shadow: 0 0 1px 1px #bfbfbf; display: inline-block;padding: 5px 20px;{{if ne .Edge "flat" }}border-radius: 25px;{{end}} }
        ...
        .badge-container .badge-box .hearts .icon {transition-duration:0.3s;fill-opacity:0;fill:{{.ReactColor}}; stroke-width:35; stroke:{{.IconColor}};}
        .badge-container .badge-box .hearts .icon:hover{transition-duration:0.3s;stroke:{{.ReactColor}};}
        .badge-container .badge-box .hearts .icon.react {transition-duration:0.3s; fill-opacity:1;stroke:{{.ReactColor}};}
        .badge-container .badge-box .text .text-box{display: table;}
        .badge-container .badge-box .text .text-box span {display: table-cell;color:{{.TextColor}};vertical-align: middle; padding-bottom: 1px;}
    ...
    <div class="badge-box">
        <div class="hearts item-container" data-service="react" onclick="onHeartEvent(this)">
            <svg xmlns="http://www.w3.org/2000/svg" width="22px" height="21px">
                <g id="heart" class="icon {{if .OnReact}}react{{end}}" transform="translate(1,0)" >
